...
const BUY = row({
  datetime: '2024-04-12T14:33:22.728Z',
  date: '2024-04-12',
...
const SAVEBACK = row({
  ...
  asset_class: 'FUND',
  name: 'MSCI ACWI USD (Acc)',
  symbol: 'IE00B6R52259',
  amount: '7.97',
  currency: 'EUR',
  description: 'Your Saveback payment',
  transaction_id: 'tr-sb-1',
});
...
describe('trade-republic-csv rows that name no security', () => {
  const HEADER_LOCAL = HEADER;
  const parseLocal = (...rows: string[]) =>
    applyProfile(readCsv([HEADER_LOCAL, ...rows].join('\n')), TRADE_REPUBLIC_CSV, {
      accountId: 'acc-tr',
    });

  it('books a reward paid before anything was bought as plain income', () => {
    // The export leaves symbol and asset_class blank on a saveback credited
    // before the owner chose what it buys. Confirmed against a real file.
    const bare = row({
      date: '2025-02-03',
      category: 'CASH',
      type: 'BENEFITS_SAVEBACK',
      ...
      currency: 'EUR',
      description: 'Your Saveback payment',
      transaction_id: 'tr-sb-bare',
    });
    const result = parseLocal(bare);
    expect(result.issues).toHaveLength(0);
    expect(result.transactions[0]?.side).toBe('income');
    expect(result.transactions[0]?.investment).toBeNull();
    expect(result.transactions[0]?.suggestedCategoryId).toBe('income-investment');
  });

  it('still reports a purchase that names no security', () => {
    const bad = row({
      date: '2025-02-03',
      category: 'TRADING',
      type: 'BUY',
      ...
      shares: '1.0000000000',
      currency: 'EUR',
      transaction_id: 'tr-buy-bare',
    });
    const result = parseLocal(bad);
    expect(result.issues).toHaveLength(1);
    expect(result.transactions).toHaveLength(1);
  });
});
